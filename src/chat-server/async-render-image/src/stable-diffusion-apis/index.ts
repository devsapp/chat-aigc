...
import { Query, Answer } from '../entity';
export const CONTROL_NET_TEMPLATE = {
    "enable_hr": false,
...
export async function genetatePrompt(payload: Query): Promise<Answer> {
...
export async function sd_models(): Promise<[]> {
    const result = await axios({
        method: 'GET',
...
export async function options(modelName: string, hash?: string) {
    const result = await axios({
        method: 'POST',
